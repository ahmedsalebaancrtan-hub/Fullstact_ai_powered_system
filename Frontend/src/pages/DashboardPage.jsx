...
export default function DashboardPage() {
  ...
  const stats = [
    { label: 'Total Assessments', value: totalQuizzes, icon: FileText, color: 'bg-blue-600', shadow: 'shadow-blue-200' },
    { label: 'Materials Processed', value: totalMaterials, icon: UploadCloud, color: 'bg-indigo-600', shadow: 'shadow-indigo-200' },
    { label: 'Active Drafts', value: recentActivity.filter(q => q.status === 'DRAFT').length, icon: BookOpen, color: 'bg-[#F8C2A0]', shadow: 'shadow-orange-100', textColor: 'text-gray-900' },
  ];
  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-black text-gray-900 tracking-tight">Academic Overview</h1>
          <p className="text-gray-500 font-medium mt-1">Welcome back. Here's what's happening with your assessments.</p>
        </div>
        <button 
          onClick={() => navigate('/upload')}
          className="flex items-center gap-3 px-8 py-4 bg-[#1e3a8a] text-white rounded-2xl font-black shadow-xl shadow-blue-900/20 hover:bg-blue-800 transition-all active:scale-95 whitespace-nowrap"
        >
          <Plus size={20} />
          Create New Quiz
        </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {stats.map((stat, index) => (
          <motion.div
            ...
            className="bg-white p-8 rounded-[32px] shadow-sm border border-gray-100 flex items-center gap-6 group hover:shadow-md transition-shadow"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 h-full">
          <div className="bg-[#1e3a8a] rounded-[40px] p-10 text-white relative overflow-hidden h-full shadow-2xl shadow-blue-900/20 flex flex-col justify-between">
          ...
          <div className="bg-white rounded-[40px] shadow-sm border border-gray-100 overflow-hidden h-full flex flex-col">
            <div className="p-8 border-b border-gray-50 flex items-center justify-between">
              ...
              <button 
                ...
                className="text-sm font-black text-[#1e3a8a] hover:text-blue-800 flex items-center gap-2 bg-blue-50 px-5 py-2.5 rounded-xl transition-all active:scale-95"
              >
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-50">
                  {recentActivity.map((quiz) => (
                    <tr key={quiz.id} className="hover:bg-gray-50/50 transition-colors group cursor-pointer" onClick={() => navigate(`/quiz-view/${quiz.id}`)}>
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-4">
                          <div className="h-10 w-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center transition-transform group-hover:scale-110 shadow-sm">
                            <FileText size={20} />
                          </div>
                          <span className="text-md font-bold text-gray-900">{quiz.title}</span>
                        </div>
                      </td>
                    ...
                    <tr>
                      <td colSpan="4" className="px-8 py-20 text-center">
                        <div className="flex flex-col items-center gap-4">
                          ...
                          <button 
                            onClick={() => navigate('/upload')}
                            className="text-[#1e3a8a] font-black text-sm hover:underline"
                          >
